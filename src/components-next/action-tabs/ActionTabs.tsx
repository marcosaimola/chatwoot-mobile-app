...
const ActionLabelTag = ({ stroke = 'black' }: { stroke?: string }) => (
...
const ActionUserIcon = ({ stroke = 'black' }: { stroke?: string }) => (
  <Svg width="29" height="28" viewBox="0 0 29 28" fill="none">
    <Path
...
const ActionStatusIcon = ({ stroke = 'black' }: { stroke?: string }) => (
...
export const ActionTabs = () => {
  ...
  const iconStroke = isDark ? '#E5E7EB' : 'black';
  ...
  const iosBg = isDark ? 'bg-[#ffffff12]' : 'bg-[#00000009]';
  const androidBg = isDark ? 'bg-gray-900' : 'bg-white';
  ...
  return (
    <ActionTabBarBackground
      ...
      style={Platform.select({
        ios: [
          tailwind.style(
            `flex flex-row rounded-[30px] items-center absolute justify-between w-[220px] px-6 py-[15px] ${iosBg}`,
        ...
        android: [
          tailwind.style(
            `flex flex-row rounded-[30px] items-center absolute justify-between w-[220px] px-6 py-[15px] ${androidBg}`,
            `h-[${ACTION_TAB_HEIGHT}px] bottom-[${bottom + 8}px] left-[${
              (SCREEN_WIDTH - 220) / 2
            }px]`,
          ),
